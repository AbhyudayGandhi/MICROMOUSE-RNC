...
void leftfollow_PID() {
    ...
    while (true) {
        ...
        bool left_wall_detected = (left_wall <= 70 && left_wall != -1);
        bool right_wall_detected = (right_wall <= 70 && right_wall != -1);
        bool front_wall_detected = (front_wall <= 70 && front_wall != -1);
...
void rightfollow_PID() {
    Serial.println("Right Wall Follower with PID started");
    ...
    while (true) {
        ...
        if (!right_wall_detected) {
            // No left wall detected, stop the robot
            Serial.println("No right wall detected. Stopping.");
