...
double kpT = 1.55 , kiT = 0.0, kdT = 0.8; //rotate in place PID constants
...
void moveForward(int distanceCm, double KP_DIST_LEFT ,double KD_DIST_LEFT, double KP_DIST_RIGHT,double KD_DIST_RIGHT,double WALL_FOLLOW_KP, double WALL_FOLLOW_KD,double KP_YAW) {
    ...
    double targetCounts = (distanceCm / (3.14 * WHEEL_DIAMETER_CM)) * COUNTS_PER_ROTATION/1.35;
    ...
    if (abs(targetYaw - readYaw())>4){
        rotateToFixed(initAngles[currentDir],20);
    }
   ...
   for(front_dist = getDistance(tofCenter);front_dist < 110 && front_dist>20;front_dist = getDistance(tofCenter)){
...
void Motor_SetSpeed(int spdL, int spdR) {
    spdL = constrain(spdL*2.2 , -255, 255);
    spdR = constrain(spdR*2.2, -255, 255);
...
void setFixedAngles() {
    ...
    initAngles[2] = wrapAngle(initial + 180);  //left back
...
void rotateInPlace(float relativeAngle, int maxSpeed) {
    ...
    while (true) {
        ...
        if (abs(error) < 0.8 && abs(yawRate) < 80.0) { // 5°/s threshold
            break;
        }
...
void Turn180() {
    ...
    int right_dis = getDistance(tofRight);
    int left_dis = getDistance(tofLeft);
