...
// For Game Rotation Vector (6-DoF, no magnetometer)
static void quaternionToEulerGRV(sh2_RotationVector_t* rv, euler_t* ypr, bool degrees = false) {
    float qr = rv->real;
    float qi = rv->i;
    float qj = rv->j;
    float qk = rv->k;

    ypr->yaw   = atan2(2.0f*(qi*qj + qk*qr), (qi*qi - qj*qj - qk*qk + qr*qr));
    ypr->pitch = asin(-2.0f*(qi*qk - qj*qr) / (qi*qi + qi*qi + qj*qj + qk*qk));
    ypr->roll  = atan2(2.0f*(qj*qk + qi*qr), (-qi*qi - qj*qj + qk*qk + qr*qr));
...
//             quaternionToEulerGI(&sensorValue.un.gyroIntegratedRV, &ypr, true);
//         }
//         // Ignore other sensor events
    
//     }
// }
void processBNOEvents() {
    // Check if the BNO sensor has been reset
    if (bno08x.wasReset()) {
        Serial.println("BNO sensor reset!");
    }
    ...
    if (bno08x.getSensorEvent(&sensorValue)) {
        // Only process the 6-DoF Game Rotation Vector (gyro + accelerometer)
        if (sensorValue.sensorId == SH2_GAME_ROTATION_VECTOR) {
            quaternionToEulerGRV(&sensorValue.un.gameRotationVector, &ypr, true);
        }
        // Ignore all other sensor events (magnetometer not used)
    }
}
...
//         // }
//         if (bno08x.getSensorEvent(&sensorValue)) {
//         // Only use gyro+accel integrated rotation vector
//             if (sensorValue.sensorId == SH2_GYRO_INTEGRATED_RV) {
//                 quaternionToEulerGI(&sensorValue.un.gyroIntegratedRV, &ypr, true);
//             }
//         // Ignore other sensor events
